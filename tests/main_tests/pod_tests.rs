...
use constmuck::{
    ...
    try_cast_slice_alt as tcsa, type_size, ImplsPod, Infer,
...
#[test]
fn cast_test() {
    must_panic(|| cast(0, infer!((ImplsPod<u32>, ImplsPod<u16>)))).unwrap();

    assert_eq!(cast::<u32, i32>(u32::MAX, infer!()), -1i32);

    unsafe {
        assert_eq!(
            cast::<u8, i8>(250, (ImplsPod::new_unchecked(), ImplsPod::new_unchecked())),
            -6i8
        );
    }
}
...
fn try_cast_test() {
    assert_eq!(
        try_cast(0, infer!((ImplsPod<u32>, ImplsPod<u16>))),
        Err(SizeMismatch)
    );
...
fn cast_ref_alt_test() {
    must_panic(|| cast_ref_alt(&0, infer!((ImplsPod<u32>, ImplsPod<u16>)))).unwrap();
...
fn try_cast_ref_alt_test() {
    assert_eq!(
        try_cast_ref_alt(&0, infer!((ImplsPod<u32>, ImplsPod<u16>))),
        Err(SizeMismatch)
    );
    assert_eq!(
...
fn cast_slice_alt_test() {
    must_panic(|| csa(&[0], infer!((ImplsPod<u32>, ImplsPod<u16>)))).unwrap();
